...
import { Plus, Trash2, Save, X, Search, Palette, Sparkles, Wand2, Download } from "lucide-react";
import html2canvas from 'html2canvas';
import { jsPDF } from 'jspdf';
import { cn } from "../lib/utils";
import { Skeleton } from "../components/ui/skeleton";
import { enhanceNoteContent, generateQuizFromNote } from "../lib/gemini";
...
export default function Notes() {
    ...
    async function handleExportPDF() {
        ...
        try {
            ...
            doc.text(currentNote.title || "Untitled Note", 20, 20);

            // Add Date
            doc.setFontSize(10);
            doc.setTextColor(100, 100, 100);
            doc.text(`Generated by Orbit Student Planner - ${new Date().toLocaleDateString()}`, 20, 30);
    ...
    function handleVoiceRecording() {
        if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
            alert("Browser kamu tidak mendukung fitur suara. Coba pakai Google Chrome.");
            return;
        }
    ...
    async function handleStartQuiz() {
        ...
        setLoading(true);
        try {
            // Simple cleanup if content is html, though current textarea is plain text
            const plainText = currentNote.content;

            const questions = await generateQuizFromNote(plainText, userProfile);
    ...
    async function handleEnhance(type) {
        ...
        setLoading(true);
        try {
            const enhancedText = await enhanceNoteContent(currentNote.content, type, userProfile);
            if (type === "summarize") {
            ...
            {isEditing ? (
                <div className={cn("flex-1 flex flex-col gap-4 rounded-xl border p-6 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-300", currentNote.color)}>
                    <div className="flex items-center gap-2">
                        <input
                            className="text-2xl font-bold bg-transparent border-none focus:outline-none placeholder:text-muted-foreground/70 flex-1"
                        ...
                        <Button
                            ...
                            className={cn(
                                ...
                                isListening ? "bg-red-500 hover:bg-red-600 text-white animate-pulse shadow-lg shadow-red-500/50" : "hover:bg-muted text-muted-foreground"
                            )}
                            title={isListening ? "Stop Recording" : "Start Voice Typing"}
                        >
                            {isListening ? <span className="text-xl">⏹️</span> : <span className="text-xl">🎙️</span>}
                        </Button>
                    </div>

                    <div className="flex gap-2 overflow-x-auto pb-2 mt-4 mb-2">
                        {COLORS.map(c => (
                            <button
                                key={c.name}
                                type="button"
                    ...
                    <div className="flex items-center gap-2 px-2 py-1 bg-muted/30 rounded-lg w-fit mb-2">
                        ...
                        <Button
                            variant="ghost"
                            size="sm"
                            className="h-7 text-xs hover:bg-green-100 hover:text-green-600 dark:hover:bg-green-900/30 font-bold"
                    ...
                    <div className="flex justify-end gap-2 pt-4 border-t border-border/50">
                        <Button variant="outline" onClick={handleExportPDF} disabled={loading || !currentNote.content} title="Export PDF">
